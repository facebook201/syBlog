...
function extrudeMeshFun(childAreaArr, mapGroup, h) {
  childAreaArr.forEach((childArea, i) => {
    ...
    mapGroup.add(mesh);
    // 伊拉克是默认发射点，颜色比其他省份颜色更亮一些进行标识
    if (childArea.name == '伊拉克') {
    ...
    meshArr.push(mesh);
    mesh.name = childArea.name;//行政区域名称
  });
}
//提取行政区域所有子区域边界数据
function childAreaArrFun(data) {
  ...
  data.features.forEach(function(childArea) {
    var area = {
      geo: [],
      // name: childArea.properties.name,//国家英文名称
      name: childArea.properties.nameZh,//国家中文名称
    };
    ...
    if (childArea.geometry.type === "Polygon") {
      ...
      childArea.geometry.coordinates[0].forEach(elem => {
        ...
        var mercator = lonLat2Mercator(elem[0], elem[1]);
        area.geo[0].push(new THREE.Vector2(mercator.x, mercator.y))
      });
      ...
      childArea.geometry.coordinates.forEach((range, index) => {
        ...
        range[0].forEach(elem => {
          ...
          var mercator = lonLat2Mercator(elem[0], elem[1]);
          area.geo[index].push(new THREE.Vector3(mercator.x, mercator.y, 0))
        });
      });
    }
  });
