var worldS = 500000;//把飞线相关柱子、光圈、棱锥全部缩放worldS倍
// var worldS = 4;//经纬度不转化为墨卡托
...
function currentCityAllFlys(start, endArr) {
  ...
  startPoint.position.z += 1.3*worldS
...
function startMeshFun(startCoord) {
  var startgeometry = new THREE.ConeBufferGeometry(0.5*worldS, 2*worldS, 4);
...
var geometryHeight = new THREE.CylinderGeometry(0.2*worldS, 0.2*worldS, 1, 6);
...
function cityPoint(cityCoord) {
  ...
  var geo = new THREE.PlaneGeometry(1.5*worldS*2, 1.5*worldS*2)
