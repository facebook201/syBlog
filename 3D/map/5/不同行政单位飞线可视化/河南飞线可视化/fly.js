var areaS = 0.15;//把飞线相关柱子、光圈、棱锥全部缩放areaS倍
...
function currentCityAllFlys(start, endArr) {
  ...
  startPoint.position.z += 1.3*areaS
...
function startMeshFun(startCoord) {
  var startgeometry = new THREE.ConeBufferGeometry(0.5*areaS, 2*areaS, 4);
...
var geometryHeight = new THREE.CylinderGeometry(0.2*areaS, 0.2*areaS, 1, 6);
...
function cityPoint(cityCoord) {
  ...
  var geo = new THREE.PlaneGeometry(1.0*areaS*2, 1.0*areaS*2)
