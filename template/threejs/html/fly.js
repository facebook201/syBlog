...
export function flyArc(radius, lon1, lat1, lon2, lat2, options) {
  ...
  // 坐标起点
  const startCoord = new Vector3(sphereCoord1.x, sphereCoord1.y, sphereCoord1.z);
  // 坐标终点
  const sphereCoord2 = lon2xyz(radius, lon2, lat2);
  const endCoord = new Vector3(sphereCoord2.x, sphereCoord2.y, sphereCoord2.z);

  const startEndQua = _3Dto2D(startCoord, endCoord);
...
function circleLine(x, y, r, startAngle, endAngle,color) {
  ...
  const points = arc.getSpacedPoints(100); //分段数50，返回51个顶点
